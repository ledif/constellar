...
void DiscordIpcClient::setActivity(const QJsonObject &activity) {
...
void DiscordIpcClient::onReadyRead() {
    ...
    while (m_readBuffer.size() >= 8) {
        ...
        const QByteArray json = m_readBuffer.mid(8, length);
...
void DiscordIpcClient::sendFrame(qint32 opcode, const QJsonObject &payload) {
